...
- [ ] **PARCIAL · K/D/A y scoreboard.** K/D/A local de Alden ya registra kills, deaths y assists desde eventos de combate; falta atribución autoritativa para 10 héroes reales.
- [x] **HECHO · Destruir el trono enemigo produce victoria/derrota funcional.**
- [x] **HECHO · Overlay terminal de victoria/derrota y bloqueo de reanudación de gameplay.**
- [ ] **P0 · Integrar el final funcional con `MatchState.phase = finished` y el flujo postpartida.**
...
- [ ] **P0 · Ampliar tests a los sistemas nuevos críticos** antes de seguir multiplicando contenido: torres, creeps, visión, items, teleport, muerte/respawn, objetivos y fin de partida.
...
- [ ] **PARCIAL · Estado K/D/A por jugador.** K/D/A local de Alden funciona, pero las estadísticas de los demás jugadores y la autoridad multi-héroe siguen pendientes.
- [x] **HECHO · Score local por equipos en el header superior**, derivado del estado disponible del scoreboard.
...
- [ ] **PENDIENTE · K/D/A genérico multi-héroe/autoritativo.**
...
- [x] **Match header con score local sincronizado con las kills disponibles.**
- [ ] **PARCIAL · K/D/A completo.** Falta la autoridad multi-héroe/10 jugadores.
- [ ] **PARCIAL · Team portraits.** Alden es real; los demás son placeholders por inicial.
- [ ] **PENDIENTE · Objective timers reales.**
- [x] **HECHO · Pantalla/overlay funcional de victoria/derrota.**
...
- [ ] **PARCIAL · Presentación final de destrucción/victoria.** Overlay funcional listo; falta acabado final premium.
...
- [ ] **PENDIENTE · Tests de victoria/derrota.**
...
- [ ] **PARCIAL · Lobby personalizada.** Ya tiene UI canónica de Custom Lobbies: browser público, crear/join por código, **confirmación explícita antes de entrar desde el browser**, sala activa, equipos visibles `DAWN / DUSK`, tamaño 1v1–5v5, código compartible y `START GAME`. El backend cubre create/join/move/leave/start, **READY server-authoritative**, chat `TEAM / ALL` con privacidad real en servidor, **spectators 0/4** que no bloquean Ready, entrada directa como espectador desde el browser cuando hay plaza, y settings host reales para Team Size, Hero Select, Bans, Allow Spectators, Public/Private y Region; los cambios de reglas de gameplay resetean Ready. El chat tiene viewport fijo con scroll y autoscroll. Faltan bots, invites directos, más mapas/modos y conexión de spectators a la sesión de juego.
- [ ] **P1 · Hero Select.**
...
- [ ] **PARCIAL · K/D/A real:** local completo; multi-héroe autoritativo pendiente.
- [x] Marcador local de kills por equipos en el header superior.
...
- [x] Destruir el trono termina funcionalmente la partida.
- [x] Overlay funcional de victoria/derrota.
- [ ] Integrar `MatchState.phase = finished` y postpartida.
- [ ] Reiniciar partida funciona.
- [ ] Tests de regresión del loop principal.

Cuando este hito esté cerrado, Dawnreach ya tendrá un **loop de partida MOBA local completo** sobre el que será mucho más seguro añadir héroes, contenido, audio premium y después networking.