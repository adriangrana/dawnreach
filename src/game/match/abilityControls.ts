...
export function createPlayableMatch(heroId = 'alden', level = 11, nowMs = 0): MatchState {
  let state = createMatchState('local-match', nowMs);
  state = addPlayerToMatch(state, { playerId: 'local-player', displayName: 'Player', team: 'dawn', slotIndex: 1 });
  state = selectHeroForPlayer(state, 'local-player', heroId);
  state = assignSelectedHeroToPlayer(state, 'local-player', LOCAL_HERO_ENTITY_ID);
  state = setHeroLevel(state, LOCAL_HERO_ENTITY_ID, level);
  const definition = getHeroDefinition(heroId);
  for (const key of ABILITY_KEYS) {
    const availableRank = definition.abilities[key].unlockLevels.filter(required => required <= level).length;
    for (let rank = 0; rank < availableRank; rank++) state = upgradeHeroAbility(state, LOCAL_HERO_ENTITY_ID, key);
  }
