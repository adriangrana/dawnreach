...
test('abandoning the last player on a team awards a connected surviving team', async () => {
  await withServer(async ({ baseUrl, port, platform }) => {
    ...
    platform.reportMatchRuntimeState(blue.body.user.id, {
      matchId: match.id,
      sequence: 1,
...
test('all explicit abandonments void a rated match instead of awarding a ghost winner', async () => {
  await withServer(async ({ baseUrl, port, platform }) => {
    const blue = await jsonFetch(`${baseUrl}/api/register`, {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ username: 'AllLeaveBlue', password: 'Iron!Crown42' }),
    });
    const red = await jsonFetch(`${baseUrl}/api/register`, {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ username: 'AllLeaveRed', password: 'Iron!Crown42' }),
    });
    const match = {
      id: 'all-explicit-abandon',
      mode: 'ranked',
      ...
      startedAt: new Date().toISOString(),
      players: [
        { userId: blue.body.user.id, username: 'AllLeaveBlue', rating: 1000, joinedAt: 1, team: 'blue', slot: 0 },
        { userId: red.body.user.id, username: 'AllLeaveRed', rating: 1000, joinedAt: 1, team: 'red', slot: 0 },
      ],
      resultToken: 'secret',
      mapSha256: null,
    };
...
test('destroying the enemy throne finishes and persists the post-match report', async () => {
  await withServer(async ({ platform }) => {
    const match = {
      id: 'throne-post-match-report',
      mode: 'normal',
      source: 'matchmaking',
      rated: false,
      status: 'in_game',
      ...
      startedAt: new Date().toISOString(),
      players: [
        { userId: 'throne-blue', username: 'Throne Blue', rating: 1000, joinedAt: 1, team: 'blue', slot: 0 },
        { userId: 'throne-red', username: 'Throne Red', rating: 1000, joinedAt: 1, team: 'red', slot: 0 },
      ],
      resultToken: 'secret',
      mapSha256: null,
    };
